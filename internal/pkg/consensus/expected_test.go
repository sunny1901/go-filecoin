...
func TestExpected_RunStateTransition_validateMining(t *testing.T) {
	...
	t.Run("passes the validateMining section when given valid mining blocks", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, emptyMessages, []block.TipSet{pTipSet}, uint64(blocks[0].ParentWeight), blocks[0].StateRoot, blocks[0].MessageReceipts)
	...
	t.Run("returns nil + mining error when election proof validation fails", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, emptyMessages, []block.TipSet{pTipSet}, uint64(blocks[0].ParentWeight), genesisBlock.StateRoot, genesisBlock.MessageReceipts)
	...
	t.Run("fails when bls signature is not valid across bls messages", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, blsMessages, emptyMessages, []block.TipSet{pTipSet}, uint64(blocks[0].ParentWeight), blocks[0].StateRoot, blocks[0].MessageReceipts)
		require.Error(t, err)
	...
	t.Run("fails when secp message has invalid signature", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, secpMessages, []block.TipSet{pTipSet}, uint64(blocks[0].ParentWeight), blocks[0].StateRoot, blocks[0].MessageReceipts)
		require.Error(t, err)
	...
	t.Run("returns nil + error when parent weight invalid", func(t *testing.T) {
		...
		_, _, err = exp.RunStateTransition(ctx, tipSet, emptyBLSMessages, emptyMessages, []block.TipSet{pTipSet}, invalidParentWeight, blocks[0].StateRoot, blocks[0].MessageReceipts)
