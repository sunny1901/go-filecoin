...
func extractGenesisBlock(source io.ReadCloser, rep repo.Repo) (*block.Block, error) {
	...
	if !cur.Parents.Equals(block.UndefTipSet.Key()) {
		...
		// the root cids of the car file are a tipset, they form the head of a chain
		...
		logInit.Infow("initialized go-filecoin with genesis file containing partial chain", "genesisCID", gensisBlk.Cid().String(), "headCIDs", headTipSet.String())
	} else {
		gensisBlk = cur
	}
